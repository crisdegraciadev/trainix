...
describe('GET /:id', () => {
  it('find by id', async () => {
    ...
    const { id: userId } = await createUser(userPayload);

    const workoutPayload = { name: 'Upper - Muscle Up', userId };
    const { id: workoutId } = await createWorkout(workoutPayload);
...
describe('GET /', () => {
  it('list with 3 elements', async () => {
    ...
    const createdWorkouts = await Promise.all(workoutPayloads.map(async (payload) => createWorkout(payload)));
...
describe('POST /', () => {
  it('create', async () => {
    ...
    const workoutPayload = { name: 'Upper - Muscle Up', userId };
    const { statusCode, body } = await request(app).post(`${BASE_WORKOUT_PATH}/`).send(workoutPayload);

    expect(statusCode).toBe(HttpStatus.CREATED);
    expect(isValidWorkoutResponse(body)).toBeTruthy();

    const { id, name, userId: workoutUserId } = body as WorkoutResponse;
    expect(name).toBe(workoutPayload.name);
    expect(workoutUserId).toBe(workoutPayload.userId);
...
describe('PUT /:id', () => {
  it('update', async () => {
    ...
    const { statusCode, body } = await request(app).put(`${BASE_WORKOUT_PATH}/${workoutId}`).send(updateWorkoutPayload);
